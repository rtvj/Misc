...
UINT8 G[12000000];
UINT8 B[12000000];
UINT8 convR[12000000];
UINT8 convG[12000000];
UINT8 convB[12000000];
...
int main(int argc, char *argv[])
{
   ...
   // clkRate_clk = ((FLOAT)cycles)/1000000.0;
	...
	sprintf(outfile, "%s_out.ppm", argv[1]);
    ...
    for(i=1; i<2999; i++)
    {
        ...
        {
            temp=0;
            temp += (PSF[0] * (FLOAT)R[((i-1)*4000)+j-1]);
            temp += (PSF[1] * (FLOAT)R[((i-1)*4000)+j]);
            temp += (PSF[2] * (FLOAT)R[((i-1)*4000)+j+1]);
            temp += (PSF[3] * (FLOAT)R[((i)*4000)+j-1]);
            temp += (PSF[4] * (FLOAT)R[((i)*4000)+j]);
            temp += (PSF[5] * (FLOAT)R[((i)*4000)+j+1]);
            temp += (PSF[6] * (FLOAT)R[((i+1)*4000)+j-1]);
            temp += (PSF[7] * (FLOAT)R[((i+1)*4000)+j]);
            temp += (PSF[8] * (FLOAT)R[((i+1)*4000)+j+1]);
	    if(temp<0.0) temp=0.0;
	    if(temp>255.0) temp=255.0;
	    convR[(i*4000)+j]=(UINT8)temp;

            temp=0;
            temp += (PSF[0] * (FLOAT)G[((i-1)*4000)+j-1]);
            temp += (PSF[1] * (FLOAT)G[((i-1)*4000)+j]);
            temp += (PSF[2] * (FLOAT)G[((i-1)*4000)+j+1]);
            temp += (PSF[3] * (FLOAT)G[((i)*4000)+j-1]);
            temp += (PSF[4] * (FLOAT)G[((i)*4000)+j]);
            temp += (PSF[5] * (FLOAT)G[((i)*4000)+j+1]);
            temp += (PSF[6] * (FLOAT)G[((i+1)*4000)+j-1]);
            temp += (PSF[7] * (FLOAT)G[((i+1)*4000)+j]);
            temp += (PSF[8] * (FLOAT)G[((i+1)*4000)+j+1]);
	    if(temp<0.0) temp=0.0;
	    if(temp>255.0) temp=255.0;
	    convG[(i*4000)+j]=(UINT8)temp;

            temp=0;
            temp += (PSF[0] * (FLOAT)B[((i-1)*4000)+j-1]);
            temp += (PSF[1] * (FLOAT)B[((i-1)*4000)+j]);
            temp += (PSF[2] * (FLOAT)B[((i-1)*4000)+j+1]);
            temp += (PSF[3] * (FLOAT)B[((i)*4000)+j-1]);
            temp += (PSF[4] * (FLOAT)B[((i)*4000)+j]);
            temp += (PSF[5] * (FLOAT)B[((i)*4000)+j+1]);
            temp += (PSF[6] * (FLOAT)B[((i+1)*4000)+j-1]);
            temp += (PSF[7] * (FLOAT)B[((i+1)*4000)+j]);
            temp += (PSF[8] * (FLOAT)B[((i+1)*4000)+j+1]);
	    if(temp<0.0) temp=0.0;
	    if(temp>255.0) temp=255.0;
	    convB[(i*4000)+j]=(UINT8)temp;
        }
    }
